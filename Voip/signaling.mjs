...
const loadBaileys = async () => {
    ...
    try {
        ...
        try {
            _baileysModule = await import("../lib/index.js");
            return _baileysModule;
        }
        catch {
            try {
...
export class SignalingBridge {
    ...
    sendTerminate = async (peerJid, callId, reason = "hangup") => {
        if (!peerJid || !callId) return;
        const routeTo = this.#toBareJid(peerJid);
        ...
        void (async () => {
            try {
                ...
                if (ackError && ackError !== "0" && ackError !== 0) {
                    ...
                    if (errStr === "404" || errStr === "480") {
                        this.#signalingErrorListener?.(signalingTag, "unreachable", effectivePeerJid, callId);
                    } else {
                        this.#signalingErrorListener?.(signalingTag, `error_${errStr}`, effectivePeerJid, callId);
                    }
                }
    ...
    #doProcessIncomingCall = async (node, voip, activeCallId) => {
        ...
        if (targetVoip) {
            switch (usableNode.tag) {
                case "offer":
                    targetVoip.handleSignalingOffer({
                        payload: b64,
    ...
    #hasConcreteDevice = (jid) => {
        ...
        return !!decoded?.user && decoded.device != null && decoded.device > 0;
    };
